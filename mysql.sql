...
--
-- مضيف: 127.0.0.1
-- وقت الجيل: 19 فبراير 2026 الساعة 01:40
...
(4, '2026-02-15 00:57:24'),
(5, '2026-02-15 01:24:42');
...
(5, 4, 2, 1, 100, 100, 100, 100.00, ''),
(6, 5, 2, 2, 5, 0, 5, 200.00, 'ثاتي'),
(7, 5, 1, 1, 15, 0, 10, 100.00, 'توا');
...
(1, 'ورد', 130, 102, 115, 100, 110, 125, 0, 0, '', 1, '2026-01-26 17:41:30'),
(2, 'غسالة ورد', 10, 0, 5, 200, 220, 250, 0, 0, '', 2, '2026-01-26 17:55:55'),
...
INSERT INTO `transfer` (`transfer_id`, `transfer_date`) VALUES
(1, '2026-02-19 01:39:20');

-- --------------------------------------------------------

--
-- بنية الجدول `transfer_of_items`
--

CREATE TABLE `transfer_of_items` (
  `transfer_of_items_id` int(11) NOT NULL,
  `transfer_of_items_transfer_id` int(11) NOT NULL,
  `transfer_of_items_items_id` int(11) NOT NULL,
  `storehouse_count` int(11) NOT NULL,
  `pos1_count` int(11) NOT NULL,
  `pos2_count` int(11) NOT NULL,
  `transfer_of_items_note` varchar(1000) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- إرجاع أو استيراد بيانات الجدول `transfer_of_items`
--

INSERT INTO `transfer_of_items` (`transfer_of_items_id`, `transfer_of_items_transfer_id`, `transfer_of_items_items_id`, `storehouse_count`, `pos1_count`, `pos2_count`, `transfer_of_items_note`) VALUES
(1, 1, 12, 100, 10, 10, '---');

-- --------------------------------------------------------

--
-- Stand-in structure for view `transfer_of_itemsview`
-- (See below for the actual view)
--
CREATE TABLE `transfer_of_itemsview` (
`transfer_of_items_id` int(11)
,`transfer_of_items_transfer_id` int(11)
,`transfer_of_items_items_id` int(11)
,`storehouse_count` int(11)
,`pos1_count` int(11)
,`pos2_count` int(11)
,`transfer_of_items_note` varchar(1000)
,`transfer_id` int(11)
...
--
-- فهارس للجدول `transfer`
--
ALTER TABLE `transfer`
  ADD PRIMARY KEY (`transfer_id`);

--
-- فهارس للجدول `transfer_of_items`
--
ALTER TABLE `transfer_of_items`
  ADD PRIMARY KEY (`transfer_of_items_id`),
  ADD KEY `transfer_of_items_transfer_id` (`transfer_of_items_transfer_id`),
  ADD KEY `transfer_of_items_items_id` (`transfer_of_items_items_id`);
...
--
ALTER TABLE `incoming_invoice_items`
  MODIFY `incoming_invoice_items_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
-- AUTO_INCREMENT for table `transfer_of_items`
--
ALTER TABLE `transfer_of_items`
  MODIFY `transfer_of_items_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
-- Structure for view `transfer_of_itemsview`
--
DROP TABLE IF EXISTS `transfer_of_itemsview`;

CREATE ALGORITHM=UNDEFINED DEFINER=`u451877070_maher`@`127.0.0.1` SQL SECURITY DEFINER VIEW `transfer_of_itemsview`  AS SELECT `transfer_of_items`.`transfer_of_items_id` AS `transfer_of_items_id`, `transfer_of_items`.`transfer_of_items_transfer_id` AS `transfer_of_items_transfer_id`, `transfer_of_items`.`transfer_of_items_items_id` AS `transfer_of_items_items_id`, `transfer_of_items`.`storehouse_count` AS `storehouse_count`, `transfer_of_items`.`pos1_count` AS `pos1_count`, `transfer_of_items`.`pos2_count` AS `pos2_count`, `transfer_of_items`.`transfer_of_items_note` AS `transfer_of_items_note`, `transfer`.`transfer_id` AS `transfer_id`, `transfer`.`transfer_date` AS `transfer_date`, `items`.`items_name` AS `items_name` FROM ((`transfer_of_items` join `transfer` on(`transfer`.`transfer_id` = `transfer_of_items`.`transfer_of_items_transfer_id`)) join `items` on(`items`.`items_id` = `transfer_of_items`.`transfer_of_items_items_id`)) ;
...
--
-- قيود الجداول `transfer_of_items`
--
ALTER TABLE `transfer_of_items`
  ADD CONSTRAINT `transfer_of_items_ibfk_1` FOREIGN KEY (`transfer_of_items_transfer_id`) REFERENCES `transfer` (`transfer_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `transfer_of_items_ibfk_2` FOREIGN KEY (`transfer_of_items_items_id`) REFERENCES `items` (`items_id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
